...
export default function Dashboard({ user }) {
    ...
    return (
        <div className="flex flex-col md:flex-row h-screen bg-background">
            ...
            >
                <h2 className="text-xl font-bold">{user == "Employer" ? "Employer" : "Freelancer"}</h2>
                <div className="mt-4">
            ...
            {/* Main content */}
            {Main && <Maincontent user={user == "Freelancer" ? "Freelancer" : "Employee"} />}
            {MyJob && <Myjob />}
            {JobPost && <PostJob />}
        </div>
    );
